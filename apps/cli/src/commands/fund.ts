...
export const registerFund = (program: Command) => {
  program
    ...
    .option("--target <mon>", "Target MON balance to wait for", "0.02")
    ...
    .action(async ({ artifact: artifactPath, delegator, target, interval, watch }: { artifact?: string; delegator?: string; target?: string; interval?: string; watch?: boolean }) => {
      ...
      const targetMon = Number(target ?? "0.02");
      if (Number.isNaN(targetMon) || targetMon <= 0) {
      ...
      console.log(`  Suggested buffer        : ${targetMon} MON`);
      console.log();
      console.log("Send MON to the address above to cover future UserOperations.");
      console.log("When the transaction confirms, rerun \`pragma status\` to verify balances.\n");

      const publicClient = createMonadPublicClient();
      const targetWei = parseEther(targetMon.toString());

      const checkBalance = async () => {
        try {
          const balance = await publicClient.getBalance({ address: delegatorAddress });
          console.log(`Current balance: ${formatEther(balance)} MON`);
          return balance;
        } catch (error) {
